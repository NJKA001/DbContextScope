...
namespace DbContextScope.Implementation
{
    ...
    /// - Keeps track of the IDbContext instances it created so that it can return the existing
    /// instance when asked for a IDbContext of a specific type.
    /// - Takes care of committing / rolling back changes and transactions on all the IDbContext
    ...
    internal class DbContextCollection : IDbContextCollection
    {
        ...
        private readonly Dictionary<IDbContext, IDbTransaction> _transactions; 
        ...
        public DbContextCollection(IDbContextFactory factory, bool readOnly = false, IsolationLevel? isolationLevel = null)
        {
            _disposed = false;
            _completed = false;

            _initializedDbContexts = new Dictionary<Type, IDbContext>();
            _transactions = new Dictionary<IDbContext, IDbTransaction>();
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException("IDbContextCollection");

            var requestedType = typeof(TDbContext);

            if (!_initializedDbContexts.ContainsKey(requestedType))
            {
                // First time we've been asked for this particular IDbContext type.
                // Create one, cache it and start its database transaction if needed.
                var dbContext = _dbContextFactory.CreateDbContext<TDbContext>(_readOnly);
        ...
        public int Commit()
        {
            if (_disposed)
                throw new ObjectDisposedException("IDbContextCollection");
            if (_completed)
                throw new InvalidOperationException("You can't call Commit() or Rollback() more than once on a IDbContextCollection. All the changes in the IDbContext instances managed by this collection have already been saved or rollback and all database transactions have been completed and closed. If you wish to make more data changes, create a new IDbContextCollection and make your changes there.");

            // Best effort. You'll note that we're not actually implementing an atomic commit 
            // here. It entirely possible that one IDbContext instance will be committed successfully
            ...
            // distributed transaction if our IDbContext instances happen to be using different 
            ...
            // IDbContext. So we should never find ourselves in a situation where two IDbContext instances
        ...
        public void Rollback()
        {
            ...
            foreach (var dbContext in _initializedDbContexts.Values)
            {
                // There's no need to explicitly rollback changes in a IDbContext as
                // IDbContext doesn't save any changes until its SaveChanges() method is called.
                // So "rolling back" for a IDbContext simply means not calling its SaveChanges()
                // method. 
        ...
        public void Dispose()
        {
            ...
            foreach (var IDbContext in _initializedDbContexts.Values)
            {
                try
                {
                    IDbContext.Dispose();
